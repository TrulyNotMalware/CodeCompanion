...
| `build.gradle.kts` | Root build: Java 25 toolchain, Kotlin 2.4.0, shared dependency versions as `extra["…"]` declarations, ktlint, shared test JVM args |
...
| `docs/` | Human-facing project documentation that is not tied to a single directory (see `docs/AGENTS.md`) |
| `gradle/` | Container for the Gradle wrapper (see `gradle/AGENTS.md`) |
