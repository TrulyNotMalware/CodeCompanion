...
class MeetingContextTest : BehaviorSpec(body = {
    ...
    } returns createSendSlackMessageEvent(isPostEventPayload = false,
        ...
        `when`("handleInteraction with successful data"){
            val interactionPayload = createInteractionPayloadInput(
                ...
                states = listOf(
                    ...
                    selectedDatePickerStates(date = LocalDate.now().plusDays(1), format = RequestMeetingContext.DATE_PATTERN),
                    selectedTimePickerStates(time = LocalTime.now(), format = RequestMeetingContext.SIMPLE_TIME_PATTERN),
