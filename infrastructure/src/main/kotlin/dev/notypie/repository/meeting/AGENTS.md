...
| `MeetingRepositoryImpl.kt` | Mapping via `toSchema` / `toDomainEntity` / `toMeetingDto`; `getMeeting` uses `throwIfSchemaNotFound`; `addParticipants` loads with participants, checks host and not canceled (`NOT_AUTHORIZED`), `startAt` in the future (`MEETING_STARTED`), trims / dedups / drops existing members and the host (`NO_NEW_PARTICIPANTS`), replays `Meeting.addParticipant` on a domain copy to enforce `MAX_PARTICIPANTS` (`OVER_CAPACITY`), then appends `ParticipantsSchema` rows and saves (`ADDED`). `getParticipants` carries a `FIXME`: it loads the whole meeting |
| `JpaMeetingRepository.kt` | JPQL reads: `findMeetingWithParticipants(meetingId)` (`JOIN FETCH`), `findAllMeetingByUserId(userId)` (publisher OR participant), `findMeetingsByUserIdAndDateRange(userId, startAt, endAt)`, `findActiveByStartAtBetween(startAt, endAt)` (`isCanceled = false`, all users), `findMeetingByUidWithParticipants(meetingUid)` (`LEFT JOIN FETCH`); `@Modifying` `updateParticipantAttendance` (keyed by `meeting.idempotencyKey` + `userId`), `existsParticipant`, `markMeetingCanceled` / `rescheduleMeeting` (`WHERE meetingUid = :meetingUid AND publisherId = :requesterId AND isCanceled = false`) |
