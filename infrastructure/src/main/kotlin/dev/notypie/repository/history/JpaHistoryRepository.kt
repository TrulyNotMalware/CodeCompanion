...
class JpaHistoryRepository(
    ...
    override fun insertNewHistory(history: History): History {
        jpaHistoryRepository.save(toPersistenceEntity(history = history))
        return history
    }

    override fun getHistoryById(id: UUID): History = toDomainEntity(jpaHistoryRepository.findById(id).orElseThrow())

    private fun toDomainEntity(historyEntity: JpaHistoryEntity): History =
        History(
            apiAppId = historyEntity.apiAppId,
            channel = historyEntity.channel,
            commandType = historyEntity.commandType,
            historyId = historyEntity.id,
            status = historyEntity.status,
            publisherId = historyEntity.publisherId,
    ...
    private fun toPersistenceEntity(history: History) =
        JpaHistoryEntity(
            ...
            apiAppId = history.apiAppId,
            channel = history.channel,
            commandType = history.commandType,
            status = history.status,
            publisherId = history.publisherId,
            commandDetailType = history.commandDetailType,
        )
}
