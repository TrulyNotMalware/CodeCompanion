...
class ModalElementBuilder {
    fun textObject(text: String, isMarkDown: Boolean): TextObject =
        if (!isMarkDown) {
            ...
            markdownTextObject(markdownText = text)
        }

    fun plainTextObject(text: String): PlainTextObject =
        plainText {
            it.text(text)
            it.emoji(true)
        }

    fun markdownTextObject(markdownText: String): MarkdownTextObject =
        markdownText {
    ...
    private fun buttonElement(
        ...
        style: ButtonType = ButtonType.DEFAULT,
    ): ButtonElement =
        ButtonElement
            .builder()
            .apply {
    ...
    fun confirmationDialogObject(
        ...
        confirmationDialog {
            it.title(plainTextObject(text = title))
            it.text(plainTextObject(text = text))
        ...
        toInteractiveObject(
            ...
            element =
                MultiUsersSelectElement
                    .builder()
                    .placeholder(plainTextObject(text = contents.placeholderText))
                    .build(),
        )

    fun plainTextInputElement(contents: TextInputContents, multiline: Boolean = false) =
    ...
    fun optionElement(text: String, description: String = "", isMarkDown: Boolean = true) =
        option {
            it.text(textObject(text = text, isMarkDown = isMarkDown))
            it.description(plainTextObject(text = description))
        }
