...
/**
 * Synchronous listener for [OpenViewEvent]. It is intentionally NOT annotated with `@Async` —
 * `views.open` requires the originating `trigger_id`, which expires 3 seconds after Slack issued
 * it. Moving to a pool thread would introduce enough latency (scheduler queueing, thread context
 * switch) to race the expiry on a loaded host.
 *
